...
struct CryptoBasicsTests {
	...
	static let records = try! VectorFile.load("crypto-basics", as: [CryptoBasicsVector].self)
		.filter {
			SwiftCryptoProvider().supportedCipherSuites.map(\.id).contains(
				$0.cipherSuite)
		}
	...
	func deriveSecret(_ record: CryptoBasicsVector) throws {
		...
		let out = try MLS.deriveSecret(
			provider, secret: record.deriveSecret.secret.bytes,
			label: record.deriveSecret.label)
	...
	func deriveTreeSecret(_ record: CryptoBasicsVector) throws {
		...
		let out = try MLS.deriveTreeSecret(
			provider, secret: v.secret.bytes, label: v.label, generation: v.generation,
			length: Int(v.length)
		)
	...
	func expandWithLabel(_ record: CryptoBasicsVector) throws {
		...
		let out = try MLS.expandWithLabel(
			provider, secret: v.secret.bytes, label: v.label, context: v.context.bytes,
			length: Int(v.length)
		)
		#expect(out == v.out.bytes)
	}
	...
	@Test(
		"encrypt_with_label: decrypts the vector's ciphertext, and our own ciphertext round-trips",
		arguments: records)
	func encryptWithLabel(_ record: CryptoBasicsVector) throws {
		...
		let decrypted = try MLS.decryptWithLabel(
			provider, privateKey: .init(v.priv.bytes), label: v.label,
			context: v.context.bytes,
		...
		let (enc, ciphertext) = try MLS.encryptWithLabel(
			provider, publicKey: .init(v.pub.bytes), label: v.label,
			context: v.context.bytes, plaintext: v.plaintext.bytes
		)
		let roundTripped = try MLS.decryptWithLabel(
			provider, privateKey: .init(v.priv.bytes), label: v.label,
			context: v.context.bytes, enc: enc, ciphertext: ciphertext
		)
		#expect(roundTripped == v.plaintext.bytes)
	}

	@Test(
		"sign_with_label: sign matches AND the vector's own signature verifies",
		arguments: records)
	func signWithLabel(_ record: CryptoBasicsVector) throws {
		...
		let ours = try MLS.signWithLabel(
			provider, privateKey: .init(v.priv.bytes), label: v.label,
			content: v.content.bytes
		)
		let oursVerifies = try MLS.verifyWithLabel(
			provider, publicKey: .init(v.pub.bytes), label: v.label,
			content: v.content.bytes, signature: ours
		)
		#expect(oursVerifies)

		let vectorVerifies = try MLS.verifyWithLabel(
			provider, publicKey: .init(v.pub.bytes), label: v.label,
			content: v.content.bytes, signature: v.signature.bytes
		)
