...
struct VarintTests {
	@Test(
		"widths change at the documented boundaries",
		arguments: [
			(UInt32(0), 1), (63, 1), (64, 2), (16_383, 2), (16_384, 4),
			(MLS.Varint.maxValue, 4),
		])
	...
	@Test(
		"round-trips across every width",
		arguments: [
			UInt32(0), 1, 63, 64, 255, 256, 16_383, 16_384, 65_535, 1 << 20,
			MLS.Varint.maxValue,
		])
	...
	func nonMinimal(bytes: [UInt8], value: UInt32) {
		#expect(
			throws: MLS.CodecError.nonMinimalLength(
				value: value, encodedBytes: bytes.count)
		) {
