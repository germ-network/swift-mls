...
extension MLS {
    ...
    public static func encryptWithLabel(
        ...
        return try provider.hpkeSeal(publicKey: publicKey, info: info, aad: nil, plaintext: plaintext)
    }
    ...
    public static func decryptWithLabel(
        ...
        let info = try EncryptContext(label: label, context: context).mlsEncoded()
        return try provider.hpkeOpen(enc: enc, secretKey: privateKey, info: info, aad: nil, ciphertext: ciphertext)
    }
}
