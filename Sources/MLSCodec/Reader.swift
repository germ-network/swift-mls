...
extension MLS {
	...
	public struct Reader: Sendable {
		...
		public mutating func readBytes(_ count: Int) throws -> [UInt8] {
			guard remaining.count >= count else {
				throw CodecError.truncated(
					needed: count, available: remaining.count)
			}
		...
		public mutating func readVarint() throws -> UInt32 {
			...
			if width > 1 {
				for byte in try readBytes(width - 1) {
		...
		public mutating func decodeVector<T: MLSDecodable>(_ type: T.Type = T.self) throws
			-> [T]
		{
			...
			while !contents.isEmpty {
				...
				guard contents.bytesRemaining < before else {
					throw CodecError.vectorNotFullyConsumed(
						remaining: contents.bytesRemaining)
				}
			}
			return values
		}
		...
		/// `optional<T>`
		public mutating func decodeOptional<T: MLSDecodable>(_ type: T.Type = T.self) throws
			-> T?
		{
