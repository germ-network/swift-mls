...
The longest-lived secret material — the key schedule's per-epoch fan-out,
retained resumption PSKs, HPKE private keys, and the KDF Extract/Expand seam
that derives them — is held in zeroizing storage (swift-crypto's
`SymmetricKey`, via the `swift-secret-bytes` package), so a derived secret is
born and kept in a buffer that scrubs on release rather than an ordinary
`Data`. This is **defense-in-depth with an unprovable ceiling**, not a
guarantee:
...
- On Apple platforms the scrubbing is CryptoKit's closed-source behavior,
  ...
  reach.
- Coverage is partial. The per-message secret store (the §9.2 consuming secret
  tree and its ratchets) and AEAD keys in flight are still ordinary `Data`;
  so are the plaintext transients at wire decode/encode boundaries. Several
  such boundaries are marked in the source as deliberate custody exits.
