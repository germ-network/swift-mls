...
- Zeroization: the longest-lived secrets (the key-schedule epoch fan-out,
  retained resumption PSKs, HPKE private keys, and the KDF derivation seam)
  are held in zeroizing storage; the shorter-lived per-message secret store is
  not yet. No test vector can detect either state. See `SECURITY.md` for the
  ceiling this buys and what it does not.
